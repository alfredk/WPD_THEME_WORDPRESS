<?php
/**
 * pergola functions and definitions
 *
 * @package pergola
 */
...
if ( ! function_exists( 'pergola_setup' ) ) :
/**
 ...
 */
function pergola_setup() {

	/*
	 * Make theme available for translation.
	 * Translations can be filed in the /languages/ directory.
	 * If you're building a theme based on pergola, use a find and replace
	 * to change 'pergola' to the name of your theme in all the template files
	 */
	load_theme_textdomain( 'pergola', get_template_directory() . '/languages' );
	...
	register_nav_menus( array(
		'primary' => __( 'Primary Menu', 'pergola' ),
	) );
	...
	add_theme_support( 'custom-background', apply_filters( 'pergola_custom_background_args', array(
...
add_action( 'after_setup_theme', 'pergola_setup' );
 ...
 */
function pergola_widgets_init() {
	register_sidebar( array(
		'name'          => __( 'Sidebar', 'pergola' ),
		'id'            => 'sidebar-1',
		'description'   => '',
...
add_action( 'widgets_init', 'pergola_widgets_init' );

/**
 * Enqueue scripts and styles.
 */
function pergola_scripts() {
  ...
  if (is_page_template('page-templates/page-nosidebar.php') || ! is_active_sidebar( 'sidebar-1' )) {
	...
	// _'s stock stuff
	wp_enqueue_style( 'pergola-style', get_stylesheet_uri(), 'null' );
	wp_enqueue_script( 'pergola-navigation', get_template_directory_uri() . '/js/navigation.js', array(), '20120206', true );
	wp_enqueue_script( 'pergola-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), '20130115', true );

	// custom js + masonry
	wp_enqueue_script( 'pergola-enquire', get_stylesheet_directory_uri() . '/js/enquire.min.js', false, '20140530', true );
	wp_enqueue_script( 'pergola-custom', get_stylesheet_directory_uri() . '/js/custom.js', array('jquery','jquery-masonry'), '20140529', true );
  ...
  // FontAwesome
  wp_enqueue_style('pergola_fontawesome', get_template_directory_uri() . '/fonts/font-awesome/css/font-awesome.min.css');
...
add_action( 'wp_enqueue_scripts', 'pergola_scripts' );

// theme editor stypes
function pergola_add_editor_styles() {
    add_editor_style( 'editor-style.css' );
}
add_action( 'init', 'pergola_add_editor_styles' );
...
function pergola_cutom_post_isFront( $query ) {
...
add_action( 'pre_get_posts', 'pergola_cutom_post_isFront' );


//CUSTOM CUSTOM POST TYPE
add_action( 'init', 'pergola_custom_postType_init' );
function pergola_custom_postType_init() {
	$labels = array(
		'name'               => 'Photos',
		'singular_name'      => 'Photo',
		'menu_name'          => 'Photos',
...
//CUSTOM TAXONOMY
add_action( 'init', 'pergola_custom_taxonomies', 0 );
function pergola_custom_taxonomies() {
// PHOTO CATEGORIES
	$labels = array(
